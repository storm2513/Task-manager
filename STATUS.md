...
* title
* shown - было ли показано уведомление
...
* send() - вызывается при запуске программы. Проверяет, есть ли уведомления, которые не были показаны (shown == False) и текущее время находится между временем начала задачи и временем за сколько нужно было напомнить. Если есть, то для этих уведомлений вызывается метод show(notification_id).
...
> Ведётся разработка основной версии приложения
